<x-layouts.app>
    <div class="flex flex-col mb-4">
        <span class="text-xl font-semibold my-2 sm:text-2xl whitespace-nowrap dark:text-white">Цілі
        </span>
        @if (session('success'))
            <x-messages.success />
    ...
    <div class="p-4 bg-white border border-gray-200 rounded-lg shadow-sm dark:border-gray-700 sm:p-6 dark:bg-gray-800">
        ...
        <ul class="flex text-sm font-medium text-center text-gray-500 divide-x divide-gray-200 rounded-lg dark:divide-gray-600 dark:text-gray-400"
            ...
            <li class="w-full">
                <button id="active_goals" data-tabs-target="#active" type="button" role="tab"
            ...
            <li class="w-full">
                <button id="completed_goals" data-tabs-target="#completed" type="button" role="tab"
            ...
            <li class="w-full">
                <button id="archived_goals" data-tabs-target="#archived" type="button" role="tab"
        ...
        <div id="fullWidthTabContent" class=" dark:border-gray-600">
            ...
            <div class="hidden" id="active" role="tabpanel" aria-labelledby="active_goals">
                ...
                <x-tables.goals-table :goals="$goals->where('status', 'active')" :accounts="$accounts" :currencies="$currencies" :sources="$sources"
            ...
            <div class="hidden" id="completed" role="tabpanel" aria-labelledby="completed_goals">
                ...
                <x-tables.goals-table :goals="$goals->where('status', 'completed')" :accounts="$accounts" :currencies="$currencies" :sources="$sources"
                    :categories="$categories" />
            </div>
            ...
            <div class="hidden" id="archived" role="tabpanel" aria-labelledby="archived_goals">
                ...
                <x-tables.goals-table :goals="$goals->where('status', 'archived')" :accounts="$accounts" :currencies="$currencies" :sources="$sources"
                    :categories="$categories" />
            </div>
            ...
            <div>
                ...
                <div class="z-50 hidden my-4 text-base list-none bg-white divide-y divide-gray-100 rounded shadow dark:bg-gray-700 dark:divide-gray-600"
                    ...
                    <div class="px-4 py-3" role="none">
                        <p class="text-sm font-medium text-gray-900 truncate dark:text-white" role="none">Sep 16,
                            2021 -
                            Sep 22, 2021</p>
                    </div>
    ...
    <div id="drawer-create-product-default"
     ...
     bg-white dark:bg-gray-800"
        ...
        <div class="relative bg-white rounded-lg dark:bg-gray-800 min-h-screen md:min-h-0">
            ...
            <x-forms.auth.form action="{{ route('goal.create') }}" method="POST">
                <div class="relative max-h-screen pb-25">

                    <x-forms.auth.input-label for="goal_name">
                        Назва цілі
                    </x-forms.auth.input-label>
                    <x-forms.auth.input-text type="text" name="goal_name" id="goal_name" />
                    <x-forms.auth.error-message :messages="$errors->get('goal_name')" class="mt-2" />

                    <x-forms.auth.input-label for="target_amount">
                        Цільова сума
                    </x-forms.auth.input-label>
                    <x-forms.auth.input-text type="number" name="target_amount" id="target_amount" />
                    <x-forms.auth.error-message :messages="$errors->get('target_amount')" class="mt-2" />

                    <x-forms.auth.input-label for="current_amount">
                        Зібрана сума
                    </x-forms.auth.input-label>
                    <x-forms.auth.input-text type="number" name="current_amount" id="current_amount"
                        value="0" />
                        ...
                        <select name="currency" id="currency"
                            ...
                            @foreach ($currencies as $currency)
                                <option value="{{ $currency->currency_id }}"
                                    {{ old('currency') == $currency->currency_id ? 'selected' : '' }}>
                                    {{ $currency->code }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <x-forms.auth.input-label for="deadline">
                        Дедлайн (Кінцева дата)
                    </x-forms.auth.input-label>
                    <x-forms.auth.input-text type="date" name="deadline" id="deadline" />
                    <x-forms.auth.error-message :messages="$errors->get('deadline')" class="mt-2" />

                    <x-forms.auth.input-label for="priority">
                        Пріорітет цілі
                    </x-forms.auth.input-label>
                    <div class="items-center mb-1 block sm:flex">

                        <x-forms.auth.input-checkbox class="mr-1 ml-1" type="radio" name="priority"
                            id="priority_low" value="low" :checked="old('priority') === 'low'" />
                        <x-forms.auth.input-label for="low">
                            Низький
                        </x-forms.auth.input-label>

                        <x-forms.auth.input-checkbox class="mr-1 ml-5" type="radio" name="priority"
                            id="priority_medium" value="medium" :checked="old('priority') === 'medium'" />
                        <x-forms.auth.input-label for="medium">
                            Середній
                        </x-forms.auth.input-label>

                        <x-forms.auth.input-checkbox class="mr-1 ml-5" type="radio" name="priority"
                            id="priority_high" value="high" :checked="old('priority') === 'high'" />
                        <x-forms.auth.input-label for="high">
                            Високий
                        </x-forms.auth.input-label>
                    </div>
    ...
    {{-- Drawer-menu --}}
    <x-forms.product-forms.del-item action="/goal/{{ $goal->goal_id ?? '#' }}" />

    <x-forms.product-forms.update-item :accounts="$accounts" :currencies="$currencies" :sources="$sources" :categories="$categories"
        :goal="$goals" />
</x-layouts.app>
