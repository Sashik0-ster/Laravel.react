    ...
    <div class="w-full mb-5">
        <div class="flex flex-wrap items-center w-full justify-between mb-5 gap-4">
            @foreach ($accounts_cards as $accounts_card)
                <x-accounts-card :title="$accounts_card->name" :account_type="$accounts_card->type" :balance="$accounts_card->balance" :currency="$accounts_card->currency->code">
                </x-accounts-card>
            @endforeach
        </div>

        <div class="w-full flex justify-end mt-6">
            {{ $accounts_cards->links('components.accounts-card-pagination') }}
        </div>
    </div>
