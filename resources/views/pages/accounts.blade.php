<x-layouts.app>
    <div class="flex flex-col">
        <div class="flex flex-wrap items-center w-full justify-between mb-5 gap-4">
            <span class="text-xl font-semibold my-2 sm:text-2xl whitespace-nowrap dark:text-white">Рахунки</span>
    ...
    <div class="w-full mb-5">
        <div class="flex flex-wrap items-center w-full mb-5 gap-4">
            @foreach ($accounts_cards as $accounts_card)
                <x-assets.cards.accounts-card :title="$accounts_card->name" :account_type="$accounts_card->type" :balance="$accounts_card->balance" :currency="$accounts_card->currency->code">
                </x-assets.cards.accounts-card>
            @endforeach
        </div>
        <div class="w-full flex justify-end mt-6">
            {{ $accounts_cards->links('components.assets.cards.accounts-card-pagination') }}
        </div>
    </div>
    ...
    <div id="drawer-create-product-default"
        ...
        <div class="relative bg-white rounded-lg dark:bg-gray-800 min-h-screen md:min-h-0">
            ...
            <x-forms.auth.form action="{{ route('accounts.create') }}" method="POST">
                <div class="relative max-h-screen pb-25">

                    <x-forms.auth.input-label for="name">
                        Назва рахунку
                    </x-forms.auth.input-label>
                    <x-forms.auth.input-text type="text" name="name" id="name"
                        placeholder="введіть назву рахунку" />
                    <x-forms.auth.error-message :messages="$errors->get('name')" class="mt-2" />

                    <x-forms.auth.input-label for="balance">
                        Баланс
                    </x-forms.auth.input-label>
                    <x-forms.auth.input-text type="number" name="balance" id="balance"
                    ...
                    <div>
                        ...
                        <select name="currency" id="currency"
                            ...
                            @foreach ($currencies as $currency)
                                <option value="{{ $currency->currency_id }}">{{ $currency->code->value }}</option>
                            @endforeach
                        </select>
                    </div>
    ...
    <x-tables.accounts-table :accounts="$accounts" />
