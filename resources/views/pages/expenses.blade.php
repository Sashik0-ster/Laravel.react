<x-layouts.app>
    ...
    <div class="flex flex-col mb-4">
        <div class="flex flex-wrap items-center w-full justify-between mb-5 gap-4">
            <span class="text-xl font-semibold my-2 sm:text-2xl whitespace-nowrap dark:text-white">Витрати</span>
    ...
    <div id="drawer-create-product-default"
     ...
     bg-white dark:bg-gray-800"
        ...
        <div class="relative bg-white rounded-lg dark:bg-gray-800 min-h-screen md:min-h-0">
            ...
            <x-forms.auth.form action="{{ route('expense.create') }}" method="POST">
                <div class="relative max-h-screen pb-25">

                    <x-forms.auth.input-label for="amount">
                        Сума
                    </x-forms.auth.input-label>
                    <x-forms.auth.input-text type="number" name="amount" id="amount" />
                    ...
                    <x-forms.auth.input-text type="date" name="expense_date" id="expense_date" />
                    <x-forms.auth.error-message :messages="$errors->get('expense_date')" class="mt-2" />

                    <x-forms.auth.input-label for="is_recurring">
                        <input type="hidden" name="is_recurring" value="0">
                        <x-forms.auth.input-checkbox type="checkbox" name="is_recurring" id="is_recurring"
                            value="1" />
                        Регулярний платіж
                    </x-forms.auth.input-label>

                    <x-forms.auth.input-label for="description">
                        Опис
                    </x-forms.auth.input-label>
                    <textarea name="description" id="description" cols="30" rows="3"
                        class="bg-gray-50 border border-gray-300 text-gray-900
                            sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5
                            dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"></textarea>
                </div>
    ...
    <x-tables.expenses-table :expenses="$expenses" :accounts="$accounts" :currencies="$currencies" :sources="$sources"
